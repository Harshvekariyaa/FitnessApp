...
class _AiExerciseScreenState extends State<AiExerciseScreen>
  ...
  Future<void> _onMarkDone() async {
    ...
    try {
      await _updateProgress(isCompleted: 1);
      _exercises[currentIndex]['is_completed'] = 1;
  ...
  Future<void> _onSkip() async {
    setState(() => isLoading = true);
    try {
      await _updateProgress(isCompleted: 0);
      _exercises[currentIndex]['is_completed'] = 0;
      isLastExercise ? await _finishWorkout() : _goNext();
    } catch (e) {
      _showSnack('Error: $e');
    } finally {
  ...
  Future<void> _finishWorkout() async {
    try {
      ...
      final double calories  = (data['total_calories']  ?? 0).toDouble();
      final int    xp        = (data['total_xp']        ?? 0).toInt();
      final int    exercises = (data['total_exercises'] ?? 0).toInt();

      if (!mounted) return;
      _showCompleteDialog(calories: calories, xp: xp, exercises: exercises);
    } catch (e) {
  ...
  // ─── Navigation ───────────────────────────────────────────────────────────
  void _goNext() {
    if (currentIndex < _exercises.length - 1) {
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => Navigator.push(
              context,
              MaterialPageRoute(
                  builder: (_) => InjuryDetailedScreen(injuryid: 12)),
            ),
          ),
        ],
      ),
